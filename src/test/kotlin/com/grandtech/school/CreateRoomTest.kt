...
open class CreateRoomTest {
    ...
    fun `returns 200 with created room for standard classroom`() {
        ...
        Mockito.`when`(
            schoolService.createRoom(
                "uid-school-room-1",
                Room(name = "Classroom 7A", capacity = 45, isStandardClassroom = true),
            ),
    ...
    fun `returns 200 with created specialist room`() {
        ...
        Mockito.`when`(
            schoolService.createRoom(
                "uid-school-room-2",
                Room(name = "Science Lab 1", capacity = 40, capabilityTag = RoomCapabilityTag.SCIENCE_LAB, isStandardClassroom = false),
            ),
        ).thenReturn(
            Room(
                id                  = "generated-uuid-2",
    ...
    fun `without auth header returns 401`() {
        given()
            .contentType(ContentType.JSON)
            .body("""{"name":"Classroom 8B","capacity":40}""")
            .`when`().post("/schools/rooms")
            .then()
                .statusCode(401)
                .body("status",  `is`(401))
    ...
    fun `for non-school account returns 403`() {
        ...
        given()
            ...
            .contentType(ContentType.JSON)
            .body("""{"name":"Classroom 9C","capacity":35}""")
            .`when`().post("/schools/rooms")
            .then()
                .statusCode(200)
                .body("status",  `is`(403))
